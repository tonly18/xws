...
func (h *HeartbeatChecker) start() {
	logger.LogInfof("[HeartBeat Start] Goroutine is Running! ConnID=%+v, Address:%v", h.conn.GetConnID(), h.conn.GetRemoteAddr())
	...
	for {
		...
		case <-h.quitChan:
			ticker.Stop()
			logger.LogInfof("[HeartBeat Start] Goroutine Exit! ConnID=%+v, Address:%v", h.conn.GetConnID(), h.conn.GetRemoteAddr())
			return
		}
	}
}
...
func (h *HeartbeatChecker) Stop() {
	logger.LogInfof("[HeartBeat Stop] Stop! ConnID=%+v, Address:%v", h.conn.GetConnID(), h.conn.GetRemoteAddr())
	h.quitChan <- true
}
...
func (h *HeartbeatChecker) check() error {
	...
	if !h.conn.IsAlive() {
		logger.LogInfof(`[HeartBeat check] remote connection %s is not alive, stop it`, h.conn.GetRemoteAddr())
		h.conn.Stop()
	}
	return nil
}
